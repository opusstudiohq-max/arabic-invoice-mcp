...
test("لا عدد حالات مكتوب بيد خارج السجلّ التاريخي", () => {
  ...
  for (const name of surfaces) {
    ...
    for (const m of text.matchAll(/(\d{2,4})\s*حالة(\s*\S+)?/g)) {
      ...
      // ادعاءُ التكافؤ (بايثون مقابل جافاسكربت) عددٌ آخر لا يخصّ هذا المقياس.
      //
      // وكان الاستثناء يقرأ ما **بعد** الرقم فقط، والعربية تقدّم المضاف:
      // «مُثبت التكافؤ على 540 حالة». فأخفق الحارس على نصٍّ صحيح — وهو خطأ
      // الحارس لا النصّ. والعدد 540 مقيسٌ من `cheque-tool/tests/parity.test.mjs`
      // بتشغيله بـ`PARITY_COUNT=1`، لا مستعادٌ من الذاكرة.
      if (/تكافؤ/.test(around)) continue;
