...
function parseTLV(buffer) {
  ...
  while (i < buffer.length) {
    ...
    // ── تشخيصُ الطول المكسور ───────────────────────────────────────────
    //
    // **كانت الأداة تُبارك أشهر عيبٍ في الميدان.** قِيس: رمزٌ أنتجه مُشفّر
    // يكتب الطول في بايتٍ واحد عند 128 بايتاً — فيضع 0x80 — كان يُصنَّف
    // «5/5 سليم». وهو خمسٌ من إحدى عشرة حزمة قِيست، وهو بعينه السبب الذي
    // يُرفض به الرمز عند الهيئة.
    //
    // و0x80 في موضع الطول ليس «128» في BER، بل **الطول غير المحدَّد** —
    // ولا يجوز هنا. فقراءتُه 128 تُخفي العيب وتُطمئن صاحبه.
    if (length === 0x80) {
      errors.push(
        `الحقل ${tag} (${TAG_NAMES[tag] || 'غير معروف'}): بايت الطول 0x80. ` +
        `القيمة 128 بايتاً لا تُكتب في بايتٍ واحد — الصواب 0x81 ثم 0x80. ` +
        `وهذا أشهر سبب لرفض الرمز، ويقع عند اسمٍ عربي من 64 حرفاً.`
      );
      break;
    }
    if (length === 0xEF && buffer[i + 2] === 0xBF && buffer[i + 3] === 0xBD) {
      errors.push(
        `الحقل ${tag} (${TAG_NAMES[tag] || 'غير معروف'}): موضعُ الطول يحمل ` +
        `EF BF BD — وهو محرف الاستبدال U+FFFD. الطول مرّ بترميز نصّي فضاع. ` +
        `ابنِ الثلاثيات بايتاتٍ لا نصّاً.`
      );
      break;
    }
    if (length > 0x82 && length !== 0xEF) {
      errors.push(
        `الحقل ${tag}: بايت الطول 0x${length.toString(16)} غير صالح — ` +
        `المسموح هنا بايتٌ دون 0x80، أو 0x81، أو 0x82.`
      );
      break;
    }
    ...
    if (length === 0x81) {
      ...
      if (length < 0x80) {
        errors.push(
          `الحقل ${tag}: طولٌ ${length} كُتب بالشكل الممتد 0x81 وهو يسع بايتاً ` +
          `واحداً. صحيحٌ بنيوياً لكنه غير معتاد — تحقّق من مُشفّرك.`
        );
      }
    } else if (length === 0x82) {
    ...
    if (length === 0 && tag <= 5) {
      errors.push(
        `الحقل ${tag} (${TAG_NAMES[tag] || 'غير معروف'}): الطول المُعلن صفر. ` +
        `يقع هذا حين يُكتب طولٌ يتجاوز 255 في بايتٍ واحد فيُبتر — ` +
        `والصواب 0x82 ثم بايتان.`
      );
      break;
    }
...
function validateZatcaQR(base64Input) {
  ...
  if (tagKeys.length === 0) {
    // **التشخيصُ الدقيق يسبق العامّ.**
    //
    // كانت هذه الرسالة تُبتلع كلَّ سبب: من يلصق رمزاً رفضته الهيئة يقرأ
    // «تأكد أن البيانات هي محتوى QR» — وهي صحيحة وعديمة النفع. والمُحلِّل
    // يعرف السبب بالضبط (بايت طولٍ 0x80، أو محرف استبدال، أو طولٌ صفر)،
    // فيُعرض هو، وتبقى العامّة حين لا يُعرف شيء.
    const diagnosis = (tlvResult.errors || [])[0];
    return {
      valid: false,
      score: 0,
      total: 5,
      ...
      fatalError: diagnosis
        || 'فشل قراءة حقول TLV — تأكد أن البيانات هي محتوى QR فاتورة ZATCA.',
    };
  }
